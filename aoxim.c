...
bool values_equal(Value a, Value b) {
  ...
  case VAL_TUPLE:
    ...
    for (size_t i = 0; i < a.tuple->size; i++) {
      if (!values_equal(a.tuple->items[i], b.tuple->items[i]))
        return false;
    }
    return true;
  case VAL_ANY:
...
Value builtin_len(Value *args, size_t argc) {
  ...
  if (args[0].type == VAL_LIST)
    return v_int(args[0].list->size);
  if (args[0].type == VAL_TUPLE)
    return v_int(args[0].tuple->size);
  if (args[0].type == VAL_STRING)
    return v_int(strlen(args[0].s));
  return v_null();
}
...
Value builtin_apply(Value *args, size_t argc) {
  if (argc != 2) {
    return v_error("apply() takes exactly 2 arguments: apply(func, args_list)");
  }

  Value fn_val = args[0];
  Value list_val = args[1];

  if (fn_val.type == VAL_ANY && fn_val.any_val)
    fn_val = *fn_val.any_val;
  if (list_val.type == VAL_ANY && list_val.any_val)
    list_val = *list_val.any_val;

  if (fn_val.type != VAL_FUNC) {
    return v_error("apply() first argument must be a function");
  }
  if (list_val.type != VAL_LIST) {
    return v_error("apply() second argument must be a list");
  }

  return call_values(fn_val.fn, list_val.list->items, list_val.list->size);
}
...
Value builtin_help(Value *args, size_t argc) {
  ...
  printf("  Supported types: int, double, string, void, ptr, long, float, "
         "char, bool, any\n");
...
Value call(Function *fn, AST **args, size_t argc, Env *caller) {
  ...
  for (size_t i = 0; i < argc; i++)
    vals[i] = eval(args[i], caller);
  for (size_t i = argc; i < effective_argc; i++)
    vals[i] = v_null();
...
Value eval(AST *a, Env *env) {
  ...
  case A_BINOP: {
    ...
    if (l.type == VAL_DOUBLE || r.type == VAL_DOUBLE) {
      ...
      if (a->bin.op == '/') {
        if (rd == 0.0)
          return v_error("division by zero");
        return v_double(ld / rd);
      }
      if (a->bin.op == '^')
...
void run_file(const char *filename) {
  ...
  while (tok.type != T_EOF) {
    ...
    if (tok.type == T_AT) {
      ...
      if (tok.type == T_PTR) {
        if (!strncmp(src, "@", 1) && !is_ident(*(src + 1))) {
          src += 1;
          current_loc.column += 1;
          tok.type = T_AT;
          strcpy(tok.text, "@");
...
int main(int argc, char **argv) {
  ...
  env_set(global_env, "int_to_ptr", v_func(make_builtin(builtin_int_to_ptr)),
          true);
  env_set(global_env, "apany", v_func(make_builtin(builtin_any)), true);
  env_set(global_env, "apply", v_func(make_builtin(builtin_apply)), true);
